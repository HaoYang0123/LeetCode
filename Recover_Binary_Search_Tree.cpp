//Leetcode 99
//出现乱序的两个数字，如果按照左中右顺序（中序遍历），那么两个数字可以按如下方式找到：
//比如：题目中的第二个例子：1，3，2，4
//那么先从左、中、右的顺序，可以找到结点2违反了规则（因为：2的左边中最大的值是3，当前值2<3）（注：需要找到最后一个违反该规则的结点）
//然后从右、中、左的顺序，又可以找到结点3违反了规则（因为：3的右边中最小的值是2，当前值3>2）（注：同上，需要找到最后一个违反该规则的结点）
//最后，将找到的两个值交换顺序
